...
//        self.emptyImage   = [UIImage imageWithContentsOfFile:pathLoadEmpty];
        self.emptyImage   = [UIImage imageNamed:@"nodata"];
...
- (void)setMaskViewStateType:(ViewStateType)maskViewStateType
{
    ...
    switch (maskViewStateType) {
        ...
        case viewStateWithEmpty:
        {
            ...
            _errorView.hidden      = YES;
            _messageLB.hidden      = NO;
            _messageLB.text        = @"咦~还没有数据呢";
            break;
        }
...
{
    [self showWithInView:view andFrame:self.frame maskViewStateType:viewStateType];
}
