...
- (instancetype)initWithDelegate:(id<MMSideslipDrawerDelegate>)delegate slipItem:(MMSideslipItem *)item;
{
    ...
    if (self)
    {
        ...
        if (item.thumbnailPath) {
            self.portraitImageView.image = [UIImage imageNamed:item.thumbnailPath];
        }
...
{
    if (!_menuView) {
        _menuView = [[UIView alloc] initWithFrame:CGRectMake(-kMMSideslipWidth, 0, kMMSideslipWidth, SCREEN_HEIGHT)];
...
{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.headView.height, kMMSideslipWidth, SCREEN_HEIGHT-self.headView.height)];
...
- (UIButton *)rankBtn
{
    if (!_rankBtn) {
        ...
        [_rankBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
