...
- (void)handleActionWithType:(IAPResultType)type data:(NSData *)data{
    ...
    switch (type) {
        ...
        case IAPResultFailed:
            NSLog(@"购买失败");
            [EasyTextView showInfoText:@"购买失败"];
            break;
        case IAPResultCancle:
            NSLog(@"用户取消购买");
            [EasyTextView showInfoText:@"已取消购买"];
            break;
        case IAPResultVerFailed:
            NSLog(@"订单校验失败");
            [EasyTextView showInfoText:@"订单校验失败"];
            break;
        ...
        case IAPResultNotArrow:
            NSLog(@"不允许程序内付费");
            [EasyTextView showInfoText:@"不允许程序内付费"];
            break;
        default:
            break;
    }
    ...
    for (SKPaymentTransaction *trans in transactions) {
        ...
        switch (trans.transactionState) {
            ...
            case SKPaymentTransactionStateFailed:
                NSLog(@"交易失败");
                [EasyTextView showInfoText:@"交易失败"];
