...
- (void)getHeightVersionAllPhotos {
    ...
    [WPFunctionView getHeightVersionAllPhotos:^(PHFetchResult *allPhotos) {
        ...
        for (NSInteger i = 0; i < allPhotos.count; i++) {
            ...
            [self.ado_collectionView registerClass:[WPAPhotoCell class] forCellWithReuseIdentifier:cellId];
...
- (void)getLowVersionAllPhotos {
    ...
    [WPFunctionView getLowVersionAllPhotos:^(ALAssetsGroup *group) {
        ...
        if (group!=nil) {
            ...
            ALAssetsGroup* allPhotoGroup = self.photoGroupArr[_photoGroupArr.count-1];
...
-(UICollectionView *)ado_collectionView
{
    if (!_ado_collectionView) {
        ...
        [layout setItemSize:CGSizeMake((SelfView_W-41)/3, (SelfView_W-41)/3)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_allPhotoArr.count) {
        ...
        WPAPhotoCell *cell = (WPAPhotoCell *)[collectionView dequeueReusableCellWithReuseIdentifier:cellId forIndexPath:indexPath];
        ...
        WPAPhotoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cellId" forIndexPath:indexPath];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WPAPhotoCell *cell = (WPAPhotoCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
-(UIButton *)finishButton
{
    if (!_finishBtn) {
        ...
        finishBut.backgroundColor = mainBlue;
        finishBut.layer.borderWidth = 0.5;
        finishBut.layer.borderColor = UIColorFromRGB(0xffffff).CGColor;
...
-(void)finishChoosePhotos:(UIButton *)finishbtn{
    ...
    if (![finishbtn.titleLabel.text isEqualToString:finishStr]&&_chooseArray.count) {
        [WPFunctionView finishChoosePhotos:^(NSMutableArray *myChoosePhotoArr) {
            self.selectPhotosBack(myChoosePhotoArr);
            [self btnClickBack];
...
-(void)makeNav {
    WPNavView *navVC = [[WPNavView alloc] init];
    ...
    if (_ado_collectionView&&_chooseArray&&_chooseCellArray) {
        if (_chooseArray.count&&_chooseCellArray.count) {
            ...
            for (NSInteger i = 0; i<_chooseCellArray.count; i++) {
                ...
                WPAPhotoCell *cell = (WPAPhotoCell *)[_ado_collectionView cellForItemAtIndexPath:indexPath];
...
{
    [_finishBtn setTitle:[NSString stringWithFormat:@"%ld/%ld 完成", (long)choosePhotoCount,(long)_selectPhotoOfMax] forState:UIControlStateNormal];
    NSString *finishStr = [NSString stringWithFormat:@"0/%ld 完成", (long)_selectPhotoOfMax];
