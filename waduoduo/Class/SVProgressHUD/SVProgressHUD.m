...
#define FontMessage  [UIFont systemFontOfSize:17]
...
- (void)updatePosition {
    ...
    if(string) {
        ...
        CGRect ceshi = [string boundingRectWithSize:CGSizeMake(200, 300) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.stringLabel.font} context:nil];
        CGSize stringSize = ceshi.size;
//        CGSize stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(200, 300)];
        ...
        CGFloat labelRectY = imageUsed ? 66 : 15;
        ...
        if (imageUsed) {
            ...
            labelRect = CGRectMake(15, labelRectY, hudWidth, stringHeight);
            hudWidth+=30;
        }
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    // 默认+0.3
    return (float)string.length*0.06 + 0.6;
}
...
+ (void)showCustom {
    [[self sharedView] showCustomProgress:-1 status:nil maskType:SVProgressHUDMaskTypeNone];
}
+ (void)showCustomWithMaskType:(SVProgressHUDMaskType)maskType {
    [[self sharedView] showCustomProgress:-1 status:nil maskType:maskType];
}
+ (void)showCustomWithStatus:(NSString*)status {
    [[self sharedView] showCustomProgress:-1 status:status maskType:SVProgressHUDMaskTypeNone];
}
+ (void)showCustomWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType {
    [[self sharedView] showCustomProgress:-1 status:status maskType:maskType];
}

- (UIImageView *)lodingImageView {
    
    if (_lodingImageView==nil) {
        NSMutableArray *array = [[NSMutableArray alloc] initWithObjects:[UIImage imageNamed:@"dropdown_loading_01"],[UIImage imageNamed:@"dropdown_loading_02"],[UIImage imageNamed:@"dropdown_loading_03"], nil];
        _lodingImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 37, 37)];
        _lodingImageView.animationImages = array; //动画图片数组
        _lodingImageView.animationDuration = 0.9; //执行一次完整动画所需的时长
        //gifImageView.animationRepeatCount = 0;  //动画重复次数 0表示无限次，默认为0
        //self.hudView.backgroundColor = [UIColor clearColor];
        _lodingImageView.center = CGPointMake(40, 40);
    }
    
    if(!_lodingImageView.superview)
        [self.hudView addSubview:_lodingImageView];
    
    return _lodingImageView;
}

- (void)showCustomProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(progress >= 0) {
        self.imageView.image = nil;
        self.imageView.hidden = NO;
        [self.lodingImageView stopAnimating];
        self.ringLayer.strokeEnd = progress;
    ...
    self.lodingImageView.centerX = self.hudView.width/2;
}
