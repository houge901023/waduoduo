...
+ (EasyEmptyView *)showErrorInView:(UIView *)superview callback:(emptyViewCallback)callback {
    ...
    [EasyLoadingView hidenLoingInView:superview]; // 隐藏加载框
    
    EasyEmptyConfig *emptyConfig = [EasyEmptyConfig shared].setTitleColor(titleC3).setScrollVerticalEnable(NO);
    EasyEmptyPart   *emptyItem = [EasyEmptyPart shared].setTitle(@"网络或服务器错误，点击重试").setImageName(@"webpage_404");
    
    NSAssert(emptyItem.buttonArray.count<3, @"you can't set more than two button") ;
    
    EasyEmptyView *emptyView;
    for (UIView *view in superview.subviews) {
        if ([view isMemberOfClass:[EasyEmptyView class]]) {
            emptyView = (EasyEmptyView *)view;
        }
    }
    
    if (emptyView == nil) {
        emptyView = [[EasyEmptyView alloc]initWithConfig:emptyConfig];
    }
...
+ (EasyEmptyView *)showEmptyInView:(UIView *)superview callback:(emptyViewCallback)callback {
    ...
    EasyEmptyConfig *emptyConfig = [EasyEmptyConfig shared].setTitleColor(titleC3).setScrollVerticalEnable(NO);
    EasyEmptyPart   *emptyItem = [EasyEmptyPart shared].setTitle(@"暂无数据").setImageName(@"default_empty");
