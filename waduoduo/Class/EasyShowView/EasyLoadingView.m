...
@property (nonatomic,strong)UIView *LoadingBgView2 ;//阴影
...
//    [superView addSubview:self];
    if (self.showConfig.cycleCornerWidth > 0) {
        ...
        if (self.showConfig.shadowColor != [UIColor clearColor]) {
            self.LoadingBgView2.frame = self.LoadingBgView.frame;
        }
...
- (UIView *)LoadingBgView2 {
    if (_LoadingBgView2 == nil) {
        _LoadingBgView2 = [[UIView alloc] init];
        _LoadingBgView2.backgroundColor = [UIColor whiteColor];
        _LoadingBgView2.layer.shadowColor = self.showConfig.shadowColor.CGColor;//阴影颜色
        _LoadingBgView2.layer.shadowOffset = CGSizeMake(0, 0);//偏移距离
        _LoadingBgView2.layer.shadowOpacity = 0.5;//不透明度
        _LoadingBgView2.layer.shadowRadius = 2.0;//半径
        _LoadingBgView2.layer.cornerRadius = self.showConfig.cycleCornerWidth;
        _LoadingBgView2.clipsToBounds = NO;
        [self addSubview:_LoadingBgView2];
        [self sendSubviewToBack:_LoadingBgView2];
    }
    return _LoadingBgView2;
}
...
+ (EasyLoadingView *)showLoadingImage {
    EasyLoadingConfig *(^configTemp)(void) = ^EasyLoadingConfig *{
        return [EasyLoadingConfig shared].setLoadingType(LoadingShowTypePlayImagesLeft).setSuperReceiveEvent(YES);
    };
    return [self showLoadingText:@"" imageName:nil config:configTemp];
}

+ (EasyLoadingView *)showLoadingImage:(NSString *)text {
    EasyLoadingConfig *(^configTemp)(void) = ^EasyLoadingConfig *{
        EasyLoadingConfig *config = [EasyLoadingConfig shared].setLoadingType(LoadingShowTypePlayImages).setSuperReceiveEvent(NO).setBgColor([UIColor whiteColor]);
        config.shadowColor = [UIColor blackColor] ;
        config.animationType = LoadingAnimationTypeNone;
        config.cycleCornerWidth = 10;
        return config;
    };
    return [self showLoadingText:text imageName:nil config:configTemp];
}
